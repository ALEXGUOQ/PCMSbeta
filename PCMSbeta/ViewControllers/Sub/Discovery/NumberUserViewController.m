...
#import "NumberUserViewController.h"
#import "PhoneBookNumberTableViewCell.h"
#import "DiscPhonesBookViewController.h"
@interface NumberUserViewController ()
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property(nonatomic, strong) NSMutableDictionary *dic;
@property(nonatomic, strong) NSArray *keyArray;
@end

@implementation NumberUserViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    NSArray *result = [CoreDataUtil retrieveData:MANAGED_OBJECT_CONTEXT modelName:@"User" withPredicate:nil];
    NSMutableDictionary *users = [[NSMutableDictionary alloc] init];
    if (result.count > 0) {
        for (User *user in result) {
            NSString *usersName = user.userName;
    ...
    if (indexPath != nil) {
        ...
        for (int i = 0; i < _keyArray.count; i++) {
            ...
            for (int j = 0; j < arr.count; j++) {
                ...
                NSString *key = [NSString stringWithFormat:@"%d%duser", i, j];
...
- (void)updateAccessibilityForCellAtIndexPath:(NSIndexPath *)indexPath
    ...
    NSString *key = [NSString stringWithFormat:@"%ld%ldcoworker", (long)indexPath.section, (long)indexPath.row];
