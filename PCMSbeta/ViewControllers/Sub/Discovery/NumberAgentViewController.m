...
#import "NumberAgentViewController.h"
#import "PhoneBookNumberTableViewCell.h"
#import "DiscPhonesBookViewController.h"
@interface NumberAgentViewController ()
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property(nonatomic, strong) NSMutableDictionary *dic;
@property(nonatomic, strong) NSArray *keyArray;
@end

@implementation NumberAgentViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"userId = %@ AND isDelete == 'N'", [[APP_DELEGATE currentUser] userId]];
    NSArray *result = [CoreDataUtil retrieveData:MANAGED_OBJECT_CONTEXT modelName:@"AgentUser" withPredicate:predicate];
    NSMutableDictionary *users = [[NSMutableDictionary alloc] init];
    if (result.count > 0) {
        for (AgentUser *user in result) {
            NSString *usersName = user.agentUserName;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    [view setBackgroundColor:UIColorFromRGB(0x59959D)];
    return view;
}
    ...
    if (indexPath != nil) {
        ...
        for (int i = 0; i < _keyArray.count; i++) {
            ...
            for (int j = 0; j < arr.count; j++) {
                ...
                NSString *key = [NSString stringWithFormat:@"%d%dagent", i, j];
